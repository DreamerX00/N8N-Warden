...
- **Do not add `--optimized` to the Keycloak command.** Keycloak dropped the `/auth` context path in v17 when it moved to Quarkus; `KC_HTTP_RELATIVE_PATH=/auth` puts it back, and *every* URL in this design depends on it — the realm's `entityId`, the ACS URL you registered in Prism, the OIDC issuer, oauth2-proxy's login/token/JWKS URLs, and nginx's `/auth/` route. But `http-relative-path` is a **build-time** option: it applies only because the container runs `start` *without* `--optimized`, so Keycloak re-augments itself at boot (most of that first-boot minute). Add `--optimized` without running `kc.sh build` first and Keycloak quietly serves at `/` again — at which point everything above 404s at once, including a Prism application whose ACS URL you cannot edit after creation.
